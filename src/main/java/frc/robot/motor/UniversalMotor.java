...
import frc.robot.config.UniversalConfig.PIDConfig;
...
public class UniversalMotor {
	...
	private PIDController builtinPID;
	...
	public UniversalMotor(MotorBrand brand, int deviceId) {
		...
		this.builtinPID = new PIDController(0, 0, 0);
	...
	public void configure(UniversalConfig config) {
		PIDConfig pidConfig = config.pidConfig;

		if (pidConfig.continuous) builtinPID.enableContinuousInput(0, Math.PI * 2);
		else builtinPID.disableContinuousInput();

		builtinPID.setPID(pidConfig.kP, pidConfig.kI, pidConfig.kD);
	...
	public void setVoltage(double voltage) {
		if (isFollower) {
			throw new IllegalStateException("Cannot set the voltage of a follower motor.");
		}
	...
	/**
	 * @return The absolute position of this motor in radians.
	 */
	...
	/**
	 * @return The angular velocity of this motor in radians per second.
	 */
